...
check: build
	-export _R_CHECK_CRAN_INCOMING_REMOTE_=FALSE && R CMD check --as-cran pivotaltrackR_$(VERSION).tar.gz
	rm -rf pivotaltrackR.Rcheck/

release: build
	-R CMD check --as-cran pivotaltrackR_$(VERSION).tar.gz
	rm -rf pivotaltrackR.Rcheck/
...
publish-pkgdown:
	rm -rf ../nealrichardson.github.io/static/r/pivotaltrackR/
	mkdir ../nealrichardson.github.io/static/r/pivotaltrackR/
	cp -r docs/* ../nealrichardson.github.io/static/r/pivotaltrackR/
